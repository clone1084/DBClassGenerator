...
namespace DBClassGenerator
{
    internal static class DbDataClassGenerator
    {
        ...
        public static void GenerateClasses(string[] args)
        {
            ...
            // --- Class generation in background task ---
            var generationTask = Task.Run(() =>
            {
                ...
                foreach (var tableName in tables)
                {
                    ...
                    var classPath = Path.Combine(outputDirectory, $"AutoGeneratedClasses");
                    Directory.CreateDirectory(classPath);

                    File.WriteAllText(Path.Combine(classPath, $"{NormalizeName(tableName)}.table.cs"), classCode);
                    Console.WriteLine($"    Generated: {NormalizeName(tableName)}.cs");

                    var extensionPath = Path.Combine(outputDirectory, $"CustomizableClasses");
                    Directory.CreateDirectory(extensionPath);

                    string extensionFileName = Path.Combine(extensionPath, $"{NormalizeName(tableName)}.custom.cs");
        ...
        static string GenerateClassCode(string table, List<ColumnInfo> columns, OracleConnection conn)
        {
            ...
            sb.AppendLine($"    [GeneratedCode(\"{nameof(DbDataClassGenerator)}\", \"{Assembly.GetExecutingAssembly().GetName().Version}\")]");            
